...
import recipeMessage from '../models/postMessage.js';
...
export const getPosts = async (req, res) => {
    ...
    try {
        ...
        const posts = await recipeMessage.find().sort({ _id: -1 }).limit(LIMIT).skip(startIndex);
...
export const getPostsBySearch = async (req, res) => {
    ...
    try {
        ...
        const posts = await recipeMessage.find({ $or: [ { title }, { tags: { $in: tags.split(',') } } ]});
...
export const getPostsByCreator = async (req, res) => {
    ...
    try {
        const posts = await recipeMessage.find({ name });
...
export const createPost = async (req, res) => {
    ...
    const newPostMessage = new recipeMessage({ ...post, creator: req.userId, createdAt: new Date().toISOString() })
